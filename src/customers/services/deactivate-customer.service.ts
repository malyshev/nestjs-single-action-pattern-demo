...
export class DeactivateCustomerService {
  ...
  private async handleCrossCuttingConcerns(customer: Customer): Promise<void> {
    await Promise.allSettled([
      this.auditService.logSystemAction('customers.deactivate', {
        customerId: customer.id,
      }),
      this.analyticsService.trackUserDeactivation(customer.id),
      this.mailingService.sendUserDeactivated(customer.id, customer.email),
      this.notificationsService.sendAccountDeactivatedNotification(customer.id),
    ]);
  }
}
